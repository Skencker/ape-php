...
<?php 
    ...
    if(!empty($_GET['id'])) {
        ...
        $statement = $db->prepare("SELECT * FROM membres_ape where id = ?");
    ...
    if(!empty($_POST)) {
        ...
        $db = Database::connect();
        $statementDelete = $db->prepare("DELETE FROM membres_ape WHERE id = ?");
...
<html lang="fr">
    ...
    <body>
        ...
        <h1>Supprimer le membre de l'ape</h1>
              <br>
              <form action="delete_membre_ape.php" method="post" class="form" role="form">
